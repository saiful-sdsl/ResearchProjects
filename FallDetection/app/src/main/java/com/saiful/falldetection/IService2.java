...
//            Log.d("Z:", ""+fusedOrientation[2]);
            ...
            //**********Sensing Danger**********
            final double SMV = Math.sqrt(accel[0] * accel[0] + accel[1] * accel[1] + accel[2] * accel[2]);
//                Log.d("SMV:", ""+SMV);
            if (SMV > 25) {
                if (sentRecently == 'N') {
...
//                    Log.d("Degrees:", "" + degreeFloat);
                    if (degreeFloat > 30 || degreeFloat2 > 30) {
                        ...
                        handler.post(new Runnable() {
                            ...
                            @Override
                            public void run() {
                                //Toast.makeText(IService2.this.getApplicationContext(), "Sensed Danger! Detected Fall Event!!!", Toast.LENGTH_SHORT).show();
                                Log.d("Service", "Sensed Danger! Detected Fall Event!!!");
                                sendMessage(FALL_EVENT, degreeFloat, degreeFloat2, SMV);
                            }
                        });
...
//                                textMsg = "Sensed Danger here => "+"http://maps.google.com/?q=<"+latitude+">,<"+longitude+">";
                            ...
                            if (phoneNum != prevNumber && phoneNum != null) {
                                ...
                                Log.d("Service", "onSensorChanged: " + sendCount);
...
//                        }
                        ...
                        handler.post(new Runnable() {
                            @Override
                            public void run() {
                                Log.d("Service", "Sudden Movement! But looks safe: " + sendCount);
                                //Toast.makeText(IService2.this.getApplicationContext(), "Sudden Movement! But looks safe [updated]", Toast.LENGTH_SHORT).show();
                                sendMessage(FAKE_FALL_EVENT, degreeFloat, degreeFloat2, SMV);
                            }
                        });
                        sendCount++;
                    }
    ...
    public void sendMessage(int event, float gyroOrientation1, float gyroOrientation2, double accelerometerVector) {
        Message message = Message.obtain();
        Bundle bundle = new Bundle();
        bundle.putFloat("sensor1", gyroOrientation1);
        bundle.putFloat("sensor2", gyroOrientation2);
        bundle.putDouble("sensor3", accelerometerVector);
